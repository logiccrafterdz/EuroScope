...
async def _seed_trades(storage, trades):
    """Helper: insert closed trades into DB."""
    for t in trades:
        sig_id = await storage.save_signal(
        ...
        await storage.update_signal_status(sig_id, "closed", pnl_pips=t["pnl"])
...
class TestBasicMetrics:
    ...
    @pytest.mark.asyncio
    async def test_empty_trades(self, analytics):
        snap = await analytics.calculate()
        assert snap.total_trades == 0
        assert snap.win_rate == 0

    @pytest.mark.asyncio
    async def test_single_win(self, analytics):
        await _seed_trades(analytics.storage, [
    ...
    @pytest.mark.asyncio
    async def test_win_loss_mix(self, analytics):
        await _seed_trades(analytics.storage, [
            {"direction": "BUY", "entry": 1.09, "pnl": 40},
    ...
    @pytest.mark.asyncio
    async def test_profit_factor(self, analytics):
        await _seed_trades(analytics.storage, [
...
class TestAdvancedMetrics:
    ...
    @pytest.mark.asyncio
    async def test_sharpe_ratio_insufficient_data(self):
        assert PerformanceAnalytics._sharpe_ratio([10]) == 0.0

    @pytest.mark.asyncio
    async def test_sortino_positive_only(self):
        pnls = [10, 20, 15]  # All positive → inf
        sortino = PerformanceAnalytics._sortino_ratio(pnls)
        assert sortino == float("inf")

    @pytest.mark.asyncio
    async def test_sortino_with_losses(self):
        pnls = [10, -5, 20, -10, 15]
        sortino = PerformanceAnalytics._sortino_ratio(pnls)
        assert sortino > 0
...
class TestSnapshotPersistence:
    ...
    @pytest.mark.asyncio
    async def test_save_and_retrieve(self, analytics):
        await _seed_trades(analytics.storage, [
            {"direction": "BUY", "entry": 1.09, "pnl": 40},
            {"direction": "SELL", "entry": 1.10, "pnl": -20},
        ])
        snap = await analytics.calculate("daily")
        snap_id = await analytics.save_snapshot(snap)
        assert snap_id > 0

        latest = await analytics.get_latest("daily")
...
class TestBreakdowns:
    ...
    @pytest.mark.asyncio
    async def test_by_strategy(self, analytics):
        await _seed_trades(analytics.storage, [
...
class TestFormatting:
    ...
    @pytest.mark.asyncio
    async def test_format_empty(self, analytics):
        snap = await analytics.calculate()
        text = analytics.format_full_report(snap)
        assert "No closed trades" in text

    @pytest.mark.asyncio
    async def test_format_with_data(self, analytics):
        await _seed_trades(analytics.storage, [
            {"direction": "BUY", "entry": 1.09, "pnl": 40},
            {"direction": "SELL", "entry": 1.10, "pnl": -20},
        ])
