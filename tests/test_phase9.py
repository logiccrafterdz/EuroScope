...
class TestTradeJournal:
    ...
    async def test_save_and_get(self, storage):
        ...
        journal = await storage.get_trade_journal()
    ...
    @pytest.mark.asyncio
    async def test_causal_chain_persisted(self, storage):
        chain = {
            "trigger": "macro_event",
        ...
        trade = await storage.get_trade_with_causal(tid)
        assert trade is not None
        assert trade["causal_chain"] == chain

    @pytest.mark.asyncio
    async def test_close_and_stats(self, storage):
        tid = await storage.save_trade_journal(
            direction="BUY", entry_price=1.0850,
            strategy="trend_following",
        )
        await storage.close_trade_journal(tid, exit_price=1.0900, pnl_pips=50.0, is_win=True)
    ...
    @pytest.mark.asyncio
    async def test_stats_by_strategy(self, storage):
        for i in range(3):
            tid = await storage.save_trade_journal(
                direction="BUY", entry_price=1.0850,
                strategy="trend_following" if i < 2 else "mean_reversion",
            )
            await storage.close_trade_journal(
    ...
    async def test_filter_by_status(self, storage):
        tid = await storage.save_trade_journal(direction="BUY", entry_price=1.08)
        await storage.save_trade_journal(direction="SELL", entry_price=1.09)

        await storage.close_trade_journal(tid, 1.09, 100.0, True)

        open_trades = await storage.get_trade_journal(status="open")
...
class TestTradeJournalSkill:
    @pytest.mark.asyncio
    async def test_log_and_close(self, storage):
        ...
        result = await skill.execute(ctx, "close_trade",
                               trade_id=trade_id, exit_price=1.09,
                               pnl_pips=50.0, is_win=True)
        assert result.success

        result = await skill.execute(ctx, "get_stats")
        assert result.success
...
class TestPatternTracker:
    @pytest.mark.asyncio
    async def test_record_and_resolve(self, storage):
        tracker = PatternTracker(storage)

        pid = await tracker.record_detection("double_bottom", "H4", "BUY", 1.0850)
        assert pid == 1

        await tracker.resolve(pid, "BUY", 1.0900, True)
    ...
    @pytest.mark.asyncio
    async def test_confidence_multiplier(self, storage):
        tracker = PatternTracker(storage)

        # Too little data → 1.0
        pid = await tracker.record_detection("head_shoulders", "H1", "SELL", 1.09)
        await tracker.resolve(pid, "SELL", 1.08, True)
        assert await tracker.get_confidence_multiplier("head_shoulders", "H1") == 1.0
        ...
        mult = await tracker.get_confidence_multiplier("head_shoulders", "H1")
        assert mult > 1.0  # 100% success → ~1.5
        assert mult <= 1.5

    @pytest.mark.asyncio
    async def test_unresolved(self, storage):
        tracker = PatternTracker(storage)
        await tracker.record_detection("triangle", "H4", "BUY", 1.09)
        await tracker.record_detection("flag", "H1", "SELL", 1.08)

        unresolved = await tracker.get_unresolved()
        assert len(unresolved) == 2

    @pytest.mark.asyncio
    async def test_format_report(self, storage):
        tracker = PatternTracker(storage)

        for _ in range(3):
            pid = await tracker.record_detection("double_bottom", "H4", "BUY", 1.08)
            await tracker.resolve(pid, "BUY", 1.09, True)

        pid = await tracker.record_detection("double_bottom", "H4", "BUY", 1.08)
        await tracker.resolve(pid, "SELL", 1.07, False)

        report = await tracker.format_report()
...
class TestAdaptiveTuner:
    @pytest.mark.asyncio
    async def test_not_enough_data(self, storage):
        tuner = AdaptiveTuner(storage)
        result = await tuner.analyze()
        assert not result["ready"]

    @pytest.mark.asyncio
    async def test_low_win_rate_recommendation(self, storage):
        tuner = AdaptiveTuner(storage)
        ...
        for _ in range(6):
            ...
            await storage.close_trade_journal(tid, 1.08, -50.0, False)
    ...
    @pytest.mark.asyncio
    async def test_high_win_rate_recommendation(self, storage):
        tuner = AdaptiveTuner(storage)
        ...
        for _ in range(6):
            ...
            await storage.close_trade_journal(tid, 1.09, 50.0, True)
    ...
    @pytest.mark.asyncio
    async def test_apply_adjustment_with_bounds(self, storage):
        tuner = AdaptiveTuner(storage)
...
class TestWorkspaceMemoryRefresh:
    @pytest.mark.asyncio
    async def test_refresh_memory(self, storage, tmp_path):
        ...
        await ws.refresh_memory(storage)
