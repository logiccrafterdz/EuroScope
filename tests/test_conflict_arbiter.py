...
class TestConflictArbiter:
    ...
    @pytest.mark.asyncio
    async def test_resolve_consensus(self, arbiter, context):
        context.metadata["technical_bias"] = "BULLISH"
        context.metadata["liquidity_signal"] = "BUY"
        context.signals["direction"] = "BUY"
        
        res = await arbiter.resolve(context)
        assert res["final_direction"] == "BUY"
        assert res["confidence"] > 0.8

    @pytest.mark.asyncio
    async def test_resolve_conflict_liquidity_wins(self, arbiter, context):
    ...
    @pytest.mark.asyncio
    async def test_session_aware_weighting(self, arbiter, context):
    ...
    @pytest.mark.asyncio
    async def test_regime_aware_weighting(self, arbiter, context):
