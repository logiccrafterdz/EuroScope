...
class TestPhase3Learning(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pattern_tracker_resolution(self):
        ...
        self.tracker.record_detection("Double Top", "H1", "Bearish", 1.0900)
        self.tracker.record_detection("Double Top", "H1", "Bearish", 1.0890)
        self.tracker.record_detection("Double Top", "H1", "Bearish", 1.0880)
        
        # 2. Check pending
        pending = self.storage.get_unresolved_patterns()
        self.assertEqual(len(pending), 3)
        
        # 3. Resolve (success)
        self.tracker.resolve_pending(1.0850) # Price dropped, success for Bearish pattern
        
        # 4. Check success rate
        rates = self.tracker.get_success_rates()
        self.assertIn("Double Top_H1", rates)
        self.assertEqual(rates["Double Top_H1"]["success_rate"], 100.0)
        
        # 5. Get multiplier
        mult = self.tracker.get_confidence_multiplier("Double Top", "H1")
    ...
    async def test_adaptive_tuner_report(self):
        ...
        for i in range(5):
            self.storage.save_trade_journal(
                direction="BUY", entry_price=1.0800 + (i * 0.0010), 
                strategy="TrendFollowing", confidence=0.7
            )
            trade_id = self.storage.get_trade_journal(status="open")[0]["id"]
            self.storage.close_trade_journal(trade_id, 1.0850 + (i * 0.0010), 50.0, True)
        
        report = self.tuner.format_report()
