...
class LLMInterface:
    ...
    async def run_proactive_analysis(self) -> dict[str, Any]:
        ...
        proactive_prompt = (
            ...
            "1. **Layer 1: Macro Catalysts & Geopolitics (HIGHEST PRIORITY)**: Focus heavily on news, wars, geopolitical tensions, speeches, or macroeconomic shifts. These dictate the market structure.\n"
            "2. **Layer 2: Liquidity Events**: Order blocks, sweeps of session highs/lows, or institutional accumulation/distribution.\n"
            "3. **Layer 3: Regime & Volatility**: Sudden deviations in volatility, trend direction changes, or notable compression/ranging.\n"
            "4. **Layer 4: Technical & Price Action**: Breakouts, bounces off key levels (Support/Resistance). Do NOT obsess over RSI or MACD unless relevant to a bigger move.\n\n"
            "ALERT PRIORITIZATION (Critical, High, Medium, Low):\n"
            "- CRITICAL: Immediate action! Breaking geopolitical news, structural market shift, or extreme high-impact event < 15m.\n"
            "- HIGH: Strong setup! Macroeconomic catalyst aligning with a technical breakout or liquidity sweep.\n"
            "- MEDIUM: Market Pulse & Insights! Pattern completing, approaching key level, or regime change confirmed.\n"
            "- LOW: Informational baseline (quiet session setup, minor levels).\n\n"
            "GUIDELINES:\n"
            "- BE PROACTIVE AND VOCAL: Share your insights! If the market is ranging/quiet, explain *why* with macro context.\n"
            "- TONE: Adopt an urgent, serious, and professional tone when macroscopic factors or news are present. Act like a high-level risk manager.\n"
            "- AVOID REPETITION: Do not just dryly recite indicators (e.g., 'RSI is 50, MACD is positive'). Synthesize the real-world events driving the chart.\n"
    ...
    async def run_periodic_observation(self) -> str:
        ...
        if not _resp or any(_resp.startswith(p) for p in ("AI unavailable", "No LLM providers", "\u274c")):
